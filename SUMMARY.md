# Phonegap Boilerplate Documentation

Documentation of Phonegap Boilerplate : https://github.com/dorian-marchal/phonegap-boilerplate

- [Setting up a project](documentation/setup.md)
- [Working with Phonegap Boilerplate](documentation/routing.md)
    + [Configure the router](documentation/router.md)
    + [Configure a Controller](documentation/controller.md)
    + [Configure a Layout](documentation/layout.md)
    + [Add a Page](documentation/page.md)
    + [`globals` singleton](documentation/globals.md)
- [Internationalization](documentation/i18n.md)
- [Authentication](documentation/auth.md)
- [Generating an icon/splashscreen](documentation/icon.md)
